...
const refreshAccessToken = async refreshToken => {
   try {
      ...
      setCookie('mahanClinic_accessToken', newAccessToken, { maxAge: 60 * 60 * 24 * 365 });
...
axiosInstance.interceptors.request.use(
   config => {
      const accessToken = getCookie('mahanClinic_accessToken');
...
axiosInstance.interceptors.response.use(
   ...
   async error => {
      console.log(error);
      const refreshToken = getCookie('mahanClinic_refreshToken');
